...
def check_online():
    ...
    all_course_payload = {'deptIds': select_course_payload['deptIds'], 'sb': 0}
    all_course_url = 'https://jwxk.ucas.ac.cn/courseManage/selectCourse?s=' + select_course_payload['s']
    page = post_data(all_course_url, data=all_course_payload)
    if page is None:
...
def select_separately(event):
    ...
    global settings
    if settings['courses_separate'] != course_list:
        settings['courses_separate'] = course_list
    ...
    for course in course_list:
        ...
        if add_course_code_to_payload(course, select_course_page):
            continue
        save_course_payload = {'deptIds': select_course_payload['deptIds'], 'sids': select_course_payload['sids']}
        save_course_url = 'https://jwxk.ucas.ac.cn/courseManage/saveCourse?s=' + select_course_payload['s']
        select_result_page = post_data(save_course_url, save_course_payload)
...
def select_together(event):
    ...
    global settings
    if settings['courses'] != course_list:
        settings['courses'] = course_list
        with open('settings.json', 'w') as fp:
            fp.write(json.dumps(settings, indent=4))
...
def login(event):
    ...
    global settings
    if settings['userName'] != user or settings['pwd'] != pwd:
    ...
    page = post_data('https://jwxk.ucas.ac.cn/login?Identity=' + iden + '&roleId=821')
    if page is None:
        login_info['text'] = '网页超时 请重新登录'
        return

    page = post_data('https://jwxk.ucas.ac.cn/courseManage/main')
    if page is None:
        login_info['text'] = '网页超时 请重新登录'
        return
...
def get_time():
    while True:
        try:
            current_time = requests.get('https://jwxk.ucas.ac.cn/courseManage/main', timeout=3)
...
def download_image_file(event):
    ...
    sess.headers['User-Agent'] = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) " \
...
if __name__ == "__main__":
    ...
    login_check = 0
    root = tkinter.Tk()
    root.title("UCAS_course_grad 1.8 by wyf && YaoBIG && viki623 QQ:751076061")
    ...
    try:
        ...
        for k, v in json.load(setting_file).items():
            settings[k] = v
        setting_file.close()
    except:
        pass
    ...
    userName = tkinter.StringVar(value=settings['userName'])
    user_input = tkinter.Entry(root, textvariable=userName)
    ...
    pwd = tkinter.StringVar(value=settings['pwd'])
    pwd_input = tkinter.Entry(root, textvariable=pwd)
    ...
    courses = tkinter.StringVar(value=settings['courses'])
    course_input = tkinter.Entry(root, textvariable=courses)
    ...
    courses_separate = tkinter.StringVar(value=settings['courses_separate'])
    course_input_separate = tkinter.Entry(root, textvariable=courses_separate)
